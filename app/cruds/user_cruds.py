...
class UserCruds:
    ...
    async def create_user(self, user: user_schemas.UserCreate) -> HTTPException:
        check = await self.check_user_by_email(user.email)
        if not check:
            db_user = users.insert().values(email=user.email, password=pbkdf2_sha256.hash(user.password),
                                            name=user.name)
    ...
    async def update_user(self, new_user: user_schemas.UserCreate) -> HTTPException:
        check = await self.check_user_by_email(new_user.email)
        if check:
            query = (users.update().where(users.c.email == new_user.email).values(
                name=new_user.name, password=pbkdf2_sha256.hash(new_user.password), email=new_user.email,
            ))
    ...
    async def delete_user(self, email: str) -> HTTPException:
        check = await self.check_user_by_email(email)
        if check:
